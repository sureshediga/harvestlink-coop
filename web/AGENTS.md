...
- **Credential email:** After Join/Invest, the certificate and ID card are emailed when `RESEND_API_KEY` or SMTP (`SMTP_HOST`, `SMTP_USER`, `SMTP_PASS`) is set. Signup still succeeds if email is missing or fails. `GET /api/health` reports `emailConfigured`.
